  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const tasks = accounts.map(async (account) => {
        ...
        if (outcome.success) {
          send({ type: 'account_done', id: account.id, newTotpSecret: outcome.newTotpSecret });
          alog.info('task | done ✓', { newTotpSecret: '***' });
          completed++;
        } else {
          send({ type: 'account_error', id: account.id, error: outcome.error });
          alog.error('task | FAILED', { err: outcome.error });
          errors++;
        }
      });
